...
### P1-5: zhenti_web 同题不同答案(2 组)

- **位置**: `src/data/raw/zhenti_web.json`
- **现象**: 同省+同年+同题型+同题干(归一化后)但答案不同:
  - 经典训练题 understand:`web-bg-049` vs `web-bg-056`
  - 经典训练题 understand:`web-bg-077` vs `web-bg-101` vs `web-bg-108`
- **影响**: 同一题在练习中出现两次,答案不一致 → 用户困惑;且破坏 `questionKey` 去重(答案不同不去重)。
- **修复建议**: 人工核对 5 条题目的答案与题干,合并为一条或修正答案后由 `mergeQuestions` 自动去重。
...
- **影响**: 该篇目中考标注/考点提示失效。
- **修复建议**: 与 learning 标题对齐(改 key 或改 learning 标题,取权威名称)。
